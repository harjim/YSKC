...
import com.yskc.docservice.models.rs.rdequipment.FullYearProjectModel;
import com.yskc.docservice.models.rs.TimeRangeModel;
import com.yskc.docservice.models.rs.rdhourconfig.RdHourConfigInfoModel;
...
public class AttDataUtils {
     ...
     * 当天最大工时
     */
    public static final BigDecimal DEFAULT_HOUR = BigDecimal.valueOf(8);
    ...
    public static List<TimeRangeModel> timeRangeStrToList(String timeRangeStr) {
        ...
        if (StringUtils.hasLength(timeRangeStr)) {
        ...
        if (!CollectionUtils.isEmpty(keyRdInfos)) {
            ...
            keyRdInfos.forEach(item -> {
                ...
                if (DateUtil.year(endDate) < year) {
                    ...
                    mBegin = DateUtil.month((begin.compareTo(yearBegin) >= 0 ? begin : yearBegin));
                    mEnd = DateUtil.month(endDate.compareTo(yearEnd) >= 0 ? yearEnd : endDate);
                }

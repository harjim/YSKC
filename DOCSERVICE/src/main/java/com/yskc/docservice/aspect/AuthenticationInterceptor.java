...
import com.yskc.docservice.enums.UserStatusEnum;
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (methodAnnotation == null) {
            try {
                ...
                String loginType = packageName.substring(packageName.lastIndexOf(".")+1);
                BaseUserInfo userInfo = null;
                if (Constant.RS_LOGIN.equals(loginType)) {
                    String companyId = request.getHeader(Constant.COMPANYID_KEY);
                    userInfo = rsUserService.getUserInfoByToken(token, companyId);
                }else if(Constant.MS_LOGIN.equals(loginType)){
                    userInfo = msUserService.getUserInfoByToken(token);
                }

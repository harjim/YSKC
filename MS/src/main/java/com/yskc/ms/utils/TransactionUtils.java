...
public class TransactionUtils implements ApplicationContextAware {
    ...
    private static DataSourceTransactionManager esDataSourceTransactionManager;
    ...
    public static TransactionStatus newTransaction(DataSourceEnum dataSourceEnum) {
        ...
        } else if (dataSourceEnum == DataSourceEnum.MS) {
            return msDataSourceTransactionManager.getTransaction(def);
        } else {
            return esDataSourceTransactionManager.getTransaction(def);
        }
    }
    ...
    public static void commit(DataSourceEnum dataSourceEnum, TransactionStatus status) {
        if (status != null) {
            ...
            } else if (dataSourceEnum == DataSourceEnum.RS) {
                rsDataSourceTransactionManager.commit(status);
            } else {
                esDataSourceTransactionManager.commit(status);
            }
        }
    }
    ...
    public static void rollback(DataSourceEnum dataSourceEnum, TransactionStatus status) {
        if (status != null) {
            if (dataSourceEnum == DataSourceEnum.MS) {
                ...
                esDataSourceTransactionManager.rollback(status);
            }
        }
    }
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        ...
        esDataSourceTransactionManager = (DataSourceTransactionManager) applicationContext.getBean(DataSourceEnum.ES.getBeanName());
    }
}
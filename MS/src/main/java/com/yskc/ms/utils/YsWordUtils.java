...
public class YsWordUtils {
    ...
    public static void createHeader(XWPFDocument doc, String headerStr, String img) throws Exception {
        //获取header
        XWPFHeader header = doc.createHeader(HeaderFooterType.DEFAULT);
        //清除存在的header
        header.clearHeaderFooter();
...
//        paragraph.setBorderBottom(Borders.THICK);
        ...
        run.addPicture(is, XWPFDocument.PICTURE_TYPE_JPEG, "pic.png", Units.toEMU(31), Units.toEMU(31));
        is.close();
    }

//    private static void createHeader(HWPFDocument doc, String header, String imgPath) throws Exception{
//        WordExtractor wordExtractor = new WordExtractor(doc);
//        Range range = doc.getHeaderStoryRange();
//        range.insertAfter(header);
//        range.insertAfter(header);
//        range.insertAfter(header);
//        range.insertAfter(header);
...
//        }
    ...
    public static void generateHeader(String sourceFile, String header, String imgPath, String outFile)throws Exception {
        try (OutputStream os = new FileOutputStream(outFile)) {
            if (sourceFile.endsWith(Constant.EXCEL_DOCX)) {
                XWPFDocument xwpfDocument = WordCache.getXWPFDocumen(sourceFile);
                createHeader(xwpfDocument, header, imgPath);
                xwpfDocument.write(os);
                xwpfDocument.close();
            }
//            else if (sourceFile.endsWith(Constant.EXCEL_DOC)) {
//                InputStream in = new FileInputStream(sourceFile);
//                HWPFDocument document = new HWPFDocument(in);
//                createHeader(document, header, imgPath);
//                document.write(os);
//            }
        } catch (Exception e) {
           throw e;

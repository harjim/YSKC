...
/**
 * @Author: hck
 * @DateTime: 2021/5/27 16:59
 * @Description: 高新技术明细
 */
@TableName("highTech_detail")
public class HighTechDetailEntity extends BaseEntity {
    @TableId
    private Integer id;
    ...
    public static HighTechDetailEntity build(HighTechDetailModel model, UserInfo userInfo, Date date,Integer highTechId,Boolean isUpdate){
        HighTechDetailEntity detailEntity=new HighTechDetailEntity();
        BeanUtils.copyProperties(model,detailEntity);
        detailEntity.setHighTechId(highTechId);
        detailEntity.setCompanyId(userInfo.getCompanyId());
        if(!isUpdate){
            detailEntity.create(userInfo.getUserId(),userInfo.getMsUserId(),date);
        }else {
            detailEntity.update(userInfo.getUserId(),userInfo.getMsUserId(),date);
        }
        return detailEntity;
    }

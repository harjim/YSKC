...
public class YsRowHandler<T> {
    ...
    public void handle(String sheetName, int sheetIndex, int rowIndex, List<Object> objList) throws Exception {
        ...
        // 超过1页且sheetName名称不是【yyyyMM】格式,抛出exit，停止读取数据并进入保存数据环节
        if (sheetIndex > 0 && !readAll) {
            throw new OwnerException(EXIT);
        }
...
//            }
            try {
                ...
                logger.error("sheet名称月份格式化失败", e);
                throw new OwnerException(MessageFormat.format("sheet【{0}】名称格式错误，格式：年月，例如：201901", sheetName));
            }
        }
        fieldMap = YsExcelUtils.checkHeaderAlias(tableField, headerAlias, requiredMap, headerList, filterMonth, tableField.getHasCol(), tableField.getHasICol(), sheetName);
    }
    ...
    public T getResult(String sheetName, int rowIndex, List<Object> objList) throws Exception {
        ...
        for (String key : objectMap.keySet()) {
            ...
            if (headerAlias.get(key) != null) {
                if (!beanFieldMap.containsKey(headerAlias.get(key))) {
                    if ((null == objectMap.get(key) || StringUtils.isEmpty(objectMap.get(key)))) {
                        colObj = 0;
                    } else {
                        if (!NumberUtil.isNumber(objectMap.get(key).toString())) {
                    ...
                    if (headerAlias.get(key).startsWith("col")) {
                        colMap.put(headerAlias.get(key), colObj);
                    } else {
                        iColMap.put(headerAlias.get(key), colObj);
                    }
                } else if (beanType.getDeclaredField(headerAlias.get(key)).getType() == Date.class && !StringUtils.isEmpty(objectMap.get(key))) {
                    Date temp = getDateTime(objectMap.get(key), key, rowBuilder, rowIndex);
                    if (null == temp) {
                        throw new OwnerException(rowBuilder.append(
                                MessageFormat.format("{0}{1}{2}", key,
                                        MapUtil.isEmpty(fieldMap.get(headerAlias.get(key))) ?
                                                "" : StringUtils.isEmpty(fieldMap.get(headerAlias.get(key)).get("sampleValue")) ?
                                                "" : fieldMap.get(headerAlias.get(key)).get("sampleValue") + "，"
                                        , "时间格式错误，请检查!")).toString());
                    }
                    recordMap.put(headerAlias.get(key), temp);
                } else {
                    if (objectMap.get(key) != null) {
                        recordMap.put(headerAlias.get(key), objectMap.get(key).toString().trim());
                    }
        ...
        if (tableField.getHasCol()) {
            loadDetail(colMap, recordMap, PAY_DETAIL, PAY);
        }
        ...
        if (rowBuilder.length() > start.length()) {
            ...
            try {
                t = YsExcelUtils.mapToPojo(recordMap, beanType);
            } catch (Exception e) {
     ...
     * @param colMap    col数据
     * @param recordMap 行
     * @param detailKey payDetail insuranceDetail
     * @param key       pay insuranceFund
     */
    private void loadDetail(Map<String, Object> colMap, Map<String, Object> recordMap, String detailKey, String key) {
        if (!CollectionUtils.isEmpty(colMap)) {
            ...
            double tempPay = 0;
            for (Object temp : colMap.values()) {
                tempPay += Double.parseDouble(temp.toString());
            }
     ...
     * @throws Exception
     */
    private Date getDateTime(Object v, String key, StringBuilder rowBuilder, int rowIndex) throws Exception {
        Date d = null;

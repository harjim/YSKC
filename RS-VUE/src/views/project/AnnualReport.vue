 ...
 * @LastEditTime: 2021-02-25 13:59:13
 * @LastEditors: ldx
 * @Description: 研发项目及费用总结报告
 * @FilePath: \RS-VUE\src\views\project\AnnualReport.vue
-->
<template>
  <a-card class="contentPage">
    <a-form layout="inline">
        ...
        <a-row>
          ...
          <a-col :span="3">
            <a-upload
              ...
              :fileList="fileList"
            >
              <a-button v-if="$auth('project:annualReport:upload')">
      ...
      <preview :filePath="filePath" :docName="fileName" v-if="$auth('project:annualReport:view')"></preview>
    </a-form>
  </a-card>
</template>
...
export default {
  ...
  methods: {
    ...
    queryFilePath () {
      this.$http.get('/sysDocument/queryDocument', { params: { year: this.currentYear, fileType: 1005, pageNo: 0, pageSize: 10 } })
        .then(res => {

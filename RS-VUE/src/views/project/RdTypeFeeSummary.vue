      ...
      >
        ...
        <a-tabs :defaultActiveKey="0">
          <template v-if="ledgerList && ledgerList.length">
            ...
            >
              <fee-summary-tab :dataMap="ledgerList[idx]" :year="currentYear" />
            </a-tab-pane>
          </template>
        </a-tabs>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      activeKey: '0',
      control: {
        search: this.$auth('project:RdTypeFeeSummary:search'),
        export: this.$auth('project:subsidiaryLedger:typeExport')
      }
    }
  },
  created () {
  ...
  methods: {
    ...
    exportData () {
      ...
      this.$exportData('/aggregation/exportDataDetail', {
        year: this.currentYear,
        exportType: 2
      }, `${this.currentYear}费用归集明细表.xls`, this.$message).then(res => {
        this.spinning = false
      })
    }
  }

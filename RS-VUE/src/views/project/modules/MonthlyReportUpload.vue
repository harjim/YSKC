      ...
      <a-form @submit="handleSubmit" :form="form">
        <a-row :gutter="24">
          <a-col :md="8" :sm="24">
            ...
            >
              <a-select
                v-decorator="['month', {rules:[{required: true, message: '请选择年份'}]}]"
...
export default {
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      if (this.form.getFieldValue('month') == null) {
        this.$message.info('请选择月份')
        return
      }
      const { form: { validateFields } } = this
      validateFields((errors, values) => {
        param.append('month', values.month)

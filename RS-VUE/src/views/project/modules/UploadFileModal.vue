      ...
      <a-form @submit="handleSubmit" :form="form">
          ...
          <a-col :md="8" :sm="24">
            ...
            >
              <a-date-picker
                placeholder
                style="width:230px"
                v-decorator="['docMonth', {rules:[{required: true, message: '请选择日期'}]}]"
              />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="24" v-if="haveMonth">
          <a-col :md="8" :sm="24">
            ...
            >
              <a-input
                v-decorator="['workshop', {rules:[{required: false, message: '请输入工艺线/车间'}]}]"
                style="width:300px"
              />
            </a-form-item>
          </a-col>
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (this.haveMonth) {
          param.append('docDate', moment(values.docMonth).format('YYYY-MM-DD'))
        }
        this.documentName = values.documentName
        param.append('documentName', values.documentName)
        if (values.workshop) {
          param.append('workshop', values.workshop)
        }
        param.append('fileType', 7004)
      })
      const config = {
        // 添加请求头

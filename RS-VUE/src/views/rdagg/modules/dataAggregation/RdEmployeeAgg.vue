        ...
        >
          <template slot="buttons">
            ...
            <span style="padding-left: 10px">
              总计：
              <a style="font-weight: 600">{{ headerData.total ? headerData.total : '-' | NumberFormat }}</a>
            </span>
            <span style="padding-left: 10px">
              工资：
              <a style="font-weight: 600">{{ headerData.pay ? headerData.pay : '-' | NumberFormat }}</a>
            </span>
            <span style="padding-left: 10px">
              五险一金：
              <a style="font-weight: 600">{{ headerData.insuranceFund ? headerData.insuranceFund : '-' | NumberFormat }}</a>
            </span>
          ...
          >
            <template slot-scope="{ row }">
              {{ row.insuranceFund ? row.insuranceFund : '-' | NumberFormat }}
            </template>
          </vxe-table-column>
          <vxe-table-column
            align="right"
          ...
          >
            <template slot-scope="{ row }">
              <span v-if="!editChecked || !row.pay">
                ...
                >
                  <template slot="title"> 差额: {{ warningField(row, 'pay', 'rdAmount') | NumberFormat }} </template>
                  <span style="color: red; font-weight: bolder">{{ row.rdPay ? Number(row.rdPay) : '-' | NumberFormat }}</span>
                </a-tooltip>
                <span v-else>{{ row.rdPay ? Number(row.rdPay) : '-' | NumberFormat }}</span>
              </span>
              <span v-else>
                <a-input-number
                  v-model="row.rdPay"
          ...
          >
            <template slot-scope="{ row }">
              ...
              >
                <template slot="title"> 差额: {{ warningField(row, 'insuranceFund', 'rdInsuranceFund') | NumberFormat }} </template>
                <span style="color: red; font-weight: bolder">{{
                  row.rdInsuranceFund ? row.rdInsuranceFund : '-' | NumberFormat
                }}</span>
              </a-tooltip>
              <span v-else>{{ row.rdInsuranceFund ? row.rdInsuranceFund : '-' | NumberFormat }}</span>
            </template>
          </vxe-table-column>
          <vxe-table-column
            align="right"

...
export default {
        ...
        proxyConfig: {
          ...
          ajax: {
            query: ({ page, sort }) => {
              ...
              const queryParams = { pageNo: page.currentPage, pageSize: page.pageSize, ...this.params }
              if (sort.field) {
                queryParams.field = sort.field
                queryParams.order = sort.order
              }
              return this.$http
                .get(this.queryUrl, { params: queryParams })
                .then(res => {
  ...
  render () {
    ...
    Object.keys(T.props).forEach(k => {
      ...
      if (k === 'pagerConfig') {
        props[k] = Object.assign(this.defaultProps[k], this[k])
        return props[k]
      }
      if (this[k]) {
        props[k] = this[k]
    ...
    return (
      <vxe-grid ref="xGrid" {...{ props, scopedSlots: { ...this.$scopedSlots }, on: this.$listeners }}>
